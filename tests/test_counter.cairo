...
use hello_starknet::counter::Counter::{ Event, CountChangedEvent };
...
fn test_count_changed_event() {
    ...
    let expected_event = Event::CountChangedEvent(
        CountChangedEvent { new_count: 10 }
    );
    assert_eq!(counter_dispatcher.get_count(), 10);
